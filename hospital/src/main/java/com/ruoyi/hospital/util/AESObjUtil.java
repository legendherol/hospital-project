...
public class AESObjUtil {
        ...
        /**
         * 加密
         * @param objByte    加密的字符串
         * @param encryptKey key值
         */
        public static byte[] encrypt(byte[] objByte, String encryptKey) throws Exception {
        //设置Cipher对象
        Cipher cipher = Cipher.getInstance(ALGORITHMS, PROVIDER);
        cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(encryptKey.getBytes(), KEY_ALGORITHM));
        //调用doFinal 转base64
        return Base64.encodeBase64(cipher.doFinal(objByte));

    }

        /**
         * 解密
         * @param objByte 解密的字符串
         * @param decryptKey 解密的key值
         */
        public static byte[] decrypt(byte[] objByte, String decryptKey) throws Exception {
        //base64格式的key字符串转byte
        byte[] decodeBase64 = Base64.decodeBase64(objByte);
        ...
        patient.setAge(15L);
        patient.setPatientName("ljl");
        String key = getKey();
        byte[] encrypt = encrypt(ByteUtil.objectToByte(patient), key);
        byte[] decrypt = decrypt(encrypt, key);
        Patient object = (Patient)ByteUtil.byteToObject(decrypt);
        System.out.println("名字:"+object.getPatientName());
